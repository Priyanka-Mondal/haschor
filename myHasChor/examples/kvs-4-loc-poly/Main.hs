...
-- | `doubleBackupReplicationStrateg` is a replication strategy that replicates the state to two backup servers.
...
-- | `doubleBackupChoreo` is a choreography that uses `doubleBackupReplicationStrateg`.
...
main = do
  [loc] <- getArgs
  case loc of
    "client" -> runChoreography config doubleBackupChoreo "client"
    "primary" -> runChoreography config doubleBackupChoreo "primary"
    "backup1" -> runChoreography config doubleBackupChoreo "backup1"
    "backup2" -> runChoreography config doubleBackupChoreo "backup2"
  return ()
  where
    config =
      mkHttpConfig
