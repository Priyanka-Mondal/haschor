...
data Request = Exit | Put String String | Get String deriving (Show, Read)
    ...
    loop stateRefs = do
      ...
      resp <- 
        client `locally` \unwrap -> do 
          putStrLn ("> " ++ show (unwrap response))
          return ((unwrap response) == (Just "Exiting..."))
      cond (client, resp) \case
        True -> do
          return ()
        False ->
          loop stateRefs
